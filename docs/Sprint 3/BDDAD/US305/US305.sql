...
BEGIN
    ...
    ELSE
        SELECT COUNT(*) AS "Found", T.INITIAL_DATE, A.ARRIVAL_DATE, T.Shipmmsi, T.InitialLocationId, A.DestinationLocationId
        ...
        FROM Arrival A
                 ...
                 INNER JOIN client Cl ON Cl.id = CC.ClientId
        WHERE C.id = f_container AND Cl.id = f_client AND CM.id = CCM.Cargo_ManifestId
        GROUP BY T.INITIAL_DATE, A.ARRIVAL_DATE, T.Shipmmsi, T.InitialLocationId, A.DestinationLocationId;
...
INNER JOIN Trip T ON T.id = A.TRIPID
WHERE T.Shipmmsi = 211331678 AND A.ARRIVAL_DATE BETWEEN TO_DATE('01-12-2020 09:00', 'DD-MM-YYYY HH24-MI') AND TO_DATE('30-12-2021 12:00', 'DD-MM-YYYY HH24-MI');